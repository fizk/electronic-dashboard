...
export default function InvertingSummingOpAmp({}: Props) {

    const [storedInputs, setStoredInputs] = useLocalStorage<State>('SummingOpAmp-inputs', [['0', '0']])
    const [storedFeedback, setStoredFeedback] = useLocalStorage<string>('SummingOpAmp-feedback', '0');
    const [storedOutput, setStoredOutput] = useLocalStorage<string>('SummingOpAmp-output', '0');

    const [inputs, setInputs] = useState<State>(storedInputs);
    const [feedback, setFeedback] = useState<string>(storedFeedback);
    const [output, setOutput] = useState<string>(storedOutput);

    const handleResistorChange = (index: number) => {
        return (event: ChangeEvent<HTMLSelectElement>) => {
            const tmpState = [...inputs];
            if (!tmpState[index]) {
                tmpState[index] = ['0', '0'];
            }
            tmpState[index] = [tmpState[index][0], event.currentTarget.value];

            updateState(tmpState, feedback);
        }
    }

    const handleVoltageChange = (index: number) => {
        return (event: ChangeEvent<HTMLInputElement>) => {
            const tmpState = [...inputs];
            if (!tmpState[index]) {
                tmpState[index] = ['0', '0'];
            }
            tmpState[index] = [event.currentTarget.value, tmpState[index][1]];

            updateState(tmpState, feedback);
        }
    }

    const handleSetFeedback = (event: ChangeEvent<HTMLSelectElement>) => {
        setFeedback(event.currentTarget.value);
        updateState(inputs, event.currentTarget.value);
    }
    ...
    const handlRemove = (index: number) => (event: MouseEvent<HTMLButtonElement>) => {
    ...
    const updateState = (state: State, feedback: string) => {
        setInputs(state);

        const isAllValuePairsSet = state.map(item => parseFloat(item.at(0)!) > 0 && parseFloat(item.at(1)!) > 0);
        const isEveryValueSet = isAllValuePairsSet.every(item => item);
        const isFeedbackSet = feedback != '' && feedback != '0' ;

        if (!isEveryValueSet || !isFeedbackSet) return;

        const outcome = calculate(state, feedback);

        setOutput(outcome.toFixed(2));

        setStoredInputs(state);
        setStoredFeedback(feedback);
        setStoredOutput(outcome.toFixed(2));
    }

    const calculate = (vr: State, feedback: string): number => {
        return vr.map(item => parseFloat(item.at(0)!) / parseFloat(item.at(1)!))
    ...
    return (
        <>
            <section className="inverting-summing-op-amp__input">
                <Section variant={['framed', 'raised']}>
                    <FormStack>
                        {inputs.map((item, index) => (
                            <FormRow key={index} variants={['stretch']}>
                                <LabelInput text={<>V<sub>{index + 1}</sub></>} type="number" value={item[0]} onChange={handleVoltageChange(index)} />
                                <ResistorSelector text={<>R<sub>{index + 1}</sub></>} value={item[1]} onSelect={handleResistorChange(index)} />
                                <Button onClick={handlRemove(index)} kind="danger">remove</Button>
                            </FormRow>
                        ))}
                        <Button onClick={handleAdd} kind="warning">add</Button>
                        <ResistorSelector text={<>R<sub>f</sub></>} value={feedback} onSelect={handleSetFeedback} />
                        <LabelOutput text={<>V<sub>out</sub></>} value={output} readOnly />
                    </FormStack>
                </Section>
            </section>
            <section className="inverting-summing-op-amp__schematic">
            ...
            <section className="inverting-summing-op-amp__content">
