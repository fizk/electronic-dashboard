...
import { Button, FormStack, FormRow } from '../../elements/Form';
import classVariant from '../../helpers/classVariant';
import isEmpty from '../../helpers/isEmpty';
import { Section, type Variants } from '../../elements/Section';
import { ResistorInput, VoltageInput } from '../../elements/ValueInput';
...
export default function VoltageDivider ({
    ...
    const handleVoutUnit = (value: string): void => updateState({
        ...internalState,
        ou: parseFloat(value),
    });

    const handleR1 = (value: string): void => updateState({
        ...internalState,
        r1: value,
    });
    const handleR1Unit = (value: string): void => updateState({
        ...internalState,
        r1u: parseFloat(value),
    });
    
    const handleR2 = (value: string): void => updateState({
        ...internalState,
        r2: value,
    });
    const handleR2Unit = (value: string): void => updateState({
        ...internalState,
        r2u: parseFloat(value),
    });
    ...
    const handleSwitch = (event: MouseEvent<HTMLButtonElement>): void => updateState({
        ...
        r1: internalState.r2,
        r2: internalState.r1,
        r1u: internalState.r2u,
        r2u: internalState.r1u,
    });
    
    const updateState = (state: State) => {
        setInternalState(state);

        if (!(!isEmpty(state.v) && !isEmpty(state.r1) && !isEmpty(state.r2))) return;

        const calculation = calculateVoltage(
            parseFloat(state.v!) * state.vu, 
            parseFloat(state.r1!) * state.r1u, 
            parseFloat(state.r2!) * state.r2u,
        );
                ...
                <FormStack>
                    ...
                    <FormRow variants={['stretch']}>
                        <ResistorInput 
                            ...
                            unit={internalState.r1u}
                            onValueChange={handleR1}
                            onUnitChange={handleR1Unit}
                        />

                        <Button onClick={handleSwitch} kind="warning">switch</Button>

                        <ResistorInput 
                            text={<>R<sub>2</sub></>}
                            value={internalState.r2}
                            unit={internalState.r2u}
                            onValueChange={handleR2}
                            onUnitChange={handleR2Unit}
                        />
                    </FormRow>
                    <FormRow variants={['stretch']}>
                        <VoltageOutput 
                            ...
                            onUnitChange={handleVoutUnit}
                        />
                    </FormRow>
                </FormStack>
            </Section>
            <footer className="voltage-divider__input">
                <details className="voltage-divider__details">
                    <summary className="voltage-divider__summary">Description</summary>
